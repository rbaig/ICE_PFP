...
### L'eix d'arquitectura de computadors en el pla d'estudis FIB-GEI
...
## Marc teòric i conceptual: referències actuals i rellevants
...
## Revisió d'experiències o pràctiques similars
...
## Enfocament metodològic i estratègies utilitzades
...
![Cicle de producció de cada unitat temàtica. Les etapes 2 i 3 són iteratives amb Claude fins a satisfer la *Definition of Done*. L'etapa 5 capitalitza l'aprenentatge per als temes següents.](figures/fig_flux_produccio.svg){#fig-flux-produccio width="80%"}
...
### Innovació en el format (Quarto Markdown i GitHub)
...
Les taules següents mostren la correspondència completa entre els materials originals (MIPS) i els nous (RISC-V), organitzades per categoria.

**Taula 1a. Teoria.** La migració de la teoria ha requerit la reescriptura íntegra dels temes relacionats amb l'ISA (T2, T3, T4, T5 i T9), on la diferència entre MIPS i RISC-V és més profunda. Els temes de jerarquia de memòria (T7 i T8) han requerit menys reescriptura de base però s'han ampliat amb continguts nous rellevants per al context tecnològic actual (GPUs, sistemes multicore, taules de pàgines multinivell).
...
**Taula 1b. Laboratori.** El canvi més visible al laboratori és la substitució de MARS per RARS com a simulador de referència. A nivell de continguts la reorganització dels continguts de teoria ha comportat el canvi de sessió d'algunes parts de les pràctiques de MIPs i s'ha aprofitat el fet de treballar amb dos arxius de C simultània ment a la sessió de coma flotant (Sessió 6) per tractar la compilació separada.
...
**Taula 1c. Problemes i solucionaris.** Tot el banc de problemes ha estat reescrit per a RISC-V. A més, s'han afegit 23 exercicis nous que cobreixen continguts sense equivalent en el material MIPS original —principalment els derivats de la modularitat de RISC-V i dels canvis estructurals del temari— i que es detallen a l'Annex B.

| Material RISC-V (nou) | Equivalent MIPS (original) | Observacions |
| :--- | :--- | :--- |
| PE.qmd — Enunciats (T1–T9) | PDF problemes MIPS | Reescrit íntegrament; 23 exercicis nous |
...
**Taula 1d. Contingut nou sense equivalent MIPS.** La modularitat de RISC-V —el disseny en extensions estàndard opcionals— ha permès introduir continguts que en una arquitectura monolítica com MIPS no tenien sentit pedagògic. El canvi de paradigma de propietari a obert ha obert també la porta a continguts de sobirania tecnològica i context industrial actual que reforcen la motivació de l'estudiantat.
...
**Taula 1e. Materials transversals.** Un resultat destacat del projecte és la creació d'una infraestructura de qualitat que fa el corpus auditable i mantenible per qualsevol membre de l'equip docent, independentment de qui hagi produït cada part. Aquests tres materials no existien en cap forma en el material original.
...
## Recollida de dades: instruments i població d'estudi
...
# Discussió, conclusions i altres propostes
...
# Annex A: Materials per a l'estudiantat {.unnumbered}

Els materials migrats estan disponibles en dos formats complementaris a través d'un punt d'entrada únic:

- **Web HTML**: accessible a la URL pública de l'assignatura (DAC). Navegable per capítols, amb cercador integrat, mode fosc/clar automàtic, disseny *responsive* i compliment de l'estàndard d'accessibilitat WCAG 2.1.
- **PDF complet**: tot el material de l'assignatura —teoria, problemes, solucionaris i laboratori— en un sol fitxer descarregable des de la mateixa URL.

Ambdós formats es generen automàticament des del repositori GitHub cada vegada que s'actualitza el material font.

# Annex B: Corpus docent migrat {.unnumbered}

## Llistat de fitxers produïts i estat de revisió {.unnumbered}
...
| `L1.qmd` | S1: Introducció al simulador RARS | Preparat per a revisió externa (pendent §Pseudoinstruccions) |
...
### Estats de revisió {.unnumbered}

- **Actiu**: document de referència transversal en ús i actualització contínua durant tota la fase de producció del material.
- **Preparat per a revisió externa**: contingut complet, codi assemblador validat al simulador RARS i lèxic harmonitzat segons `contrib.qmd`; pendent de revisió per altres membres de l'equip docent abans del desplegament.
- **En procés**: contingut parcialment migrat; pendent de completar abans del desplegament al primer quadrimestre del curs 2025-26.

### Criteris de completesa (*Definition of Done*) {.unnumbered}
...
## Exercicis nous sense equivalent en el material MIPS original {.unnumbered}
