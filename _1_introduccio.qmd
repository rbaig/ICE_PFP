...
### L'assignatura Estructura de computadors (EC) --- abans i després d'aquest projecte

#### Objectiu general

Fer que l'estudiant comprengui la frontera entre el programari i el maquinari i els mecanismes que regeixen el rendiment i la robustesa del sistema a través de l'estudi del funcionament intern d'un processador industrial 
de tipus RISC.

#### Resultats d'aprenentatge específics

Els resultats d'aprenentatge específics (RA) que es detallen a continuació són el producte directe de les decisions pedagògiques preses durant l'execució d'aquest projecte. La seva formulació segueix la taxonomia de Bloom en la seva versió revisada (Anderson & Krathwohl, 2001; Bloom et al., 1956), que classifica els objectius d'aprenentatge en sis nivells cognitius ascendents. Els RA de l'assignatura EC cobreixen deliberadament els quatre primers nivells —recordar, comprendre, aplicar i analitzar—, amb èmfasi especial en l'aplicació i l'anàlisi, que són els nivells propis d'una assignatura de segon curs d'enginyeria.
...
L'estructura dels continguts resultant de la revisió feta en aquest projecte és la següent. 

La @tbl-sequenciacio-tematica mostra la seqüenciació temàtica i temporal per a un quadrimestre de 13 setmanes resultant de la revisió i migració feta en aquest projecte. Aquesta seqüenciació ha estat dissenyada per construir progressivament els resultats d'aprenentatge específics (RA) de manera lògica i pedagògica, assegurant que els estudiants adquireixin les competències necessàries en cada etapa abans de passar a la següent. 
...
: Seqüenciació temàtica i temporal per a un quadrimestre de 13 setmanes {#tbl-sequenciacio-tematica tbl-colwidths="[5,35,55]" .striped}

En l'elaboració dels materials s'ha parat una atenció especial a no emprar mai conceptes, terminologia o notació que no s'hagin introduït prèviament.
...
L'assignatura s'avalua mitjançant una fórmula ponderada que combina les proves escrites i el laboratori. El model d'avaluació és un element de context previ al projecte i no és objecte de modificació en aquesta migració, tot i que la reflexió sobre el seu futur s'aborda a l'Annex.
...
Reavaluació: sí, cada quadrimestre.[^reavaluacio]

[^reavaluacio]: La reavaluació al GEI de la FIB és un mecanisme extraordinari perquè els estudiants amb una nota final d'entre 3,5 i 4,9 puguin aprovar una assignatura suspesa. Aquest procés inclou una preparació prèvia obligatòria consistent en sis sessions de repàs de dues hores cadascuna i assistència obligatòria, dissenyades específicament per resoldre dubtes i reforçar els conceptes clau de la matèria. Un cop finalitzat aquest bloc de dotze hores de formació intensiva, l'estudiant realitza un examen final on la qualificació màxima que es pot obtenir i que constarà a l'expedient acadèmic és un 5 (Apte).

## Descripció de les necessitats, oportunitats o problemàtiques detectades

### Necessitat: Actualització de l'arquitectura de referència

L'arquitectura **MIPS** (Microprocessor without Interlocked Pipeline Stages) va revolucionar la computació en introduir el concepte de *pipeline* sense bloquejos, una innovació radical que delegava la gestió dels conflictes de dades al compilador en lloc del maquinari. Aquesta simplicitat es traduïa en un cicle d'instrucció únic, instruccions de longitud fixa de 32 bits i una estricta estructura *Load/Store*. En conjunt, MIPS va demostrar que un maquinari simplificat, en mans d'un programari intel·ligent, era la clau per a la potència computacional moderna.

Tot i la seva importància històrica, **MIPS ha quedat obsoleta** com a **arquitectura de referència** per a l'ensenyament de l'estructura de computadors. El seu desenvolupament pràctic es va aturar fa més d'una dècada, i el seu ecosistema d'eines i recursos educatius està estancat. A més, MIPS no reflecteix les tendències actuals de la indústria ni les necessitats de recerca en àrees com la seguretat, els sistemes embeguts o la computació d'alt rendiment.

### Oportunitat 1: Migració de l'arquitectura de referència

RISC-V és un projecte d'ISA obert i lliure d'ús, sorgit de la Universitat de Califòrnia, Berkeley. Va ser ideat per alguns dels mateixos enginyers que van ser pioners en l'arquitectura MIPS, i es beneficia de més de quaranta anys d'experiència en el disseny d'arquitectures RISC. La seva llicència permissiva el distingeix d'altres arquitectures propietàries: qualsevol empresa o individu pot dissenyar, fabricar i vendre xips sense pagar drets de llicència. A més, RISC-V ha estat adoptada com a arquitectura de referència estratègica per diverses institucions i governs, incloent-hi la Unió Europea, que la veu com una eina clau per impulsar la sobirania tecnològica.

Des del punt de vista docent, RISC-V presenta avantatges clars sobre MIPS. L'ISA base (RV32I) conté menys de 50 instruccions, la qual cosa facilita la comprensió inicial dels conceptes fonamentals. A partir d'aquesta base minimalista, es poden afegir extensions de manera modular (multiplicació i divisió, coma flotant, operacions atòmiques, etc.), cosa que permet estructurar el curs de manera progressiva. A diferència de MIPS, RISC-V ha estat dissenyada des de zero tenint en compte les necessitats de la recerca i la docència modernes, evitant decisions de disseny heretades que generaven confusió als estudiants. Finalment, l'ecosistema obert d'eines —compiladors, simuladors, implementacions en maquinari— és accessible sense barreres legals ni de cost.
...
Des de la perspectiva del professor, la migració a RISC-V obre l'accés a un ecosistema de **recursos educatius moderns**. L'ecosistema RISC-V ha crescut exponencialment, amb simuladors avançats (com RARS), plaques de desenvolupament assequibles, com la Raspberry Pi Pico 2, amb doble nucli RP2350 RISC-V, per uns 5 EUR, que obre la porta a pràctiques amb maquinari real i les darreres edicions dels llibres de referència de Patterson i Hennessy, que utilitzen RISC-V com a arquitectura principal.
