...
Tal com mostra la @fig-produccio-tot, l'estratègia central ha estat la **migració iterativa i incremental**: en lloc d'abordar la totalitat del temari com un bloc monolític, s'ha treballat per unitats temàtiques funcionals, cadascuna seguint un cicle de producció que integra la IA generativa com a eina d'acceleració.

::: {#fig-produccio-tot layout="[105,-10,75]" layout-valign="center"}
![**Fases de l'Etapa de producció**. La fletxa de l'esquerra indica el grau de supervisió humana del contingut tècnic i pedagògic: és mínima a P2 (Unitats de producció) i màxima a P4 (Cicle de Revisió Externa).](figures/etapa_produccio.svg){#fig-etapa-produccio}

![**Cicle de producció de cada unitat de producció**. Les fases U2 i U3 s'alternen iterativament fins a satisfer la DoD-UP. La fase U6 capitalitza l'aprenentatge acumulat per a les unitats següents.](figures/unitat_produccio.svg){#fig-unitat-produccio}

**Etapa de producció en conjunt**. L'esquema de l'esquerra (@fig-etapa-produccio) mostra les cinc fases macro del projecte i el grau creixent de supervisió humana al llarg del procés. L'esquema de la dreta (@fig-unitat-produccio) desplega el cicle intern de cada unitat de producció (P2): les sis etapes que van des de l'extracció del material original fins al refinament dels protocols, amb la iteració U2--U3 com a mecanisme de qualitat central.
:::
...
El cicle de producció de cada unitat de producció (@fig-unitat-produccio) està format per les fases següents, que s'executen de manera seqüencial:
